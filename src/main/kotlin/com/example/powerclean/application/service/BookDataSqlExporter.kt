...
/**
 * BookData INSERT SQL 파일 생성기.
 * 수집 API 호출 시 INSERT 쿼리문을 .sql 파일로 출력한다.
 */
@Component
class BookDataSqlExporter(
    @Value("\${book-data.sql-export.dir:./sql-exports}") private val exportDir: String,
) {
    private val logger = LoggerFactory.getLogger(BookDataSqlExporter::class.java)

    /**
     * BookData 리스트를 INSERT SQL 파일로 생성.
     * @return 생성된 파일 경로 (생성된 항목이 없으면 null)
     */
    fun export(books: List<BookData>, source: String): String? {
        ...
        val fileName = "book_data_${source.lowercase()}_$timestamp.sql"
        val file = File(dir, fileName)

        val sb = StringBuilder()
        sb.appendLine("-- BookData INSERT SQL")
